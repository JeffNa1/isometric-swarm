...
func _setup_button_visuals_and_signals() -> void:
	var buttons = [
		{"btn": btn_play, "icon": "play", "color": Color(0.2, 2.8, 3.8, 1.0)},
		{"btn": btn_how_to_play, "icon": "how_to_play", "color": Color(0.2, 3.5, 1.8, 1.0)},
		{"btn": btn_settings, "icon": "settings", "color": Color(3.5, 2.4, 0.4, 1.0)},
		{"btn": btn_credits, "icon": "credits", "color": Color(3.5, 3.0, 0.6, 1.0)},
		{"btn": btn_quit, "icon": "quit", "color": Color(3.8, 0.35, 0.45, 1.0)}
	]
	...
	for b_data in buttons:
		...
		style_normal.bg_color = Color(0.06, 0.09, 0.16, 0.88)
		style_normal.border_color = Color(0.18, 0.35, 0.55, 0.75)
		style_normal.set_border_width_all(2)
		style_normal.set_corner_radius_all(6)
		style_normal.content_margin_left = 54.0
		style_normal.content_margin_right = 20.0
...
func _process(delta: float) -> void:
	...
	# Title breathing hover
	if title_box:
		var float_y = sin(time_alive * 2.2) * 4.5
		title_box.position.y = 40.0 + float_y
	...
	for b in btn_scales.keys():
		...
		# Update chevron slide
		var ch = btn_chevrons[b] as Label
		if target.x > 1.0:
			ch.position.x = lerp(ch.position.x, 6.0, 16.0 * delta)
			ch.modulate.a = lerp(ch.modulate.a, 1.0, 16.0 * delta)
		else:
			ch.position.x = lerp(ch.position.x, -16.0, 16.0 * delta)
			ch.modulate.a = lerp(ch.modulate.a, 0.0, 16.0 * delta)
